     ...
     return (
          <div className="w-full h-full flex flex-col items-start gap-y-4 border-light/70 
               ...
               <div className='size-44 md:w-7/12 md:h-[15rem] lg:h-[19rem] relative'>
                    <ThemeAwareSkeleton className='rounded-lg object-cover object-center size-full 
                         border dark:border-graySurface2' />
               </div>
               <div className="size-full flex flex-col gap-y-5">
                    <div className="flex flex-col gap-y-3">
                         {[...Array(3)].map((_, index) => (
                              <ThemeAwareSkeleton key={index}
                    ...
                    <div className="w-full flex items-center justify-center gap-x-2 md:gap-x-3">
                         {[...Array(2)].map((_, index) => (
                              <ThemeAwareSkeleton key={index} borderRadius={99}
                                   className="w-[7.5rem] md:w-40 h-10 lg:h-12 border dark:border-graySurface2" />
                         ))}
                    </div>
                    <ThemeAwareSkeleton borderRadius={99}
               ...
               <div className={`md:block hidden`}>
                    <div className="md:pt-4 flex flex-col md:border-none gap-y-2 md:gap-y-7">
                         <div className="flex items-center justify-center gap-x-20">
                              {[...Array(2)].map((_, index) => (
                                   <ThemeAwareSkeleton key={index} className="w-32 border dark:border-graySurface2" />
                              ))}
                         </div>
                         <div className="flex items-center justify-center gap-x-5 w-full">
                              {cardSkeletons.map((_, index) => (
                                   <ThemeAwareSkeleton key={index} className="h-60 md:w-64 lg:w-52 xl:w-56 
